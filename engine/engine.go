...
func New() *Engine {
	...
	eng.Register("commands", func(job *Job) Status {
		//commands()按字母顺序，返回当前注册的所有的command，
		//每一个command对应一个handler
...
func (eng *Engine) Shutdown() { //为docker daemon的关闭做善后工作,保证所有运行中的handler执行完毕
	//具体的工作如上注释
	eng.l.Lock()
	if eng.shutdown { //已经shutdown则立即返回，没有则继续执行
		eng.l.Unlock()
		return
	}
	eng.shutdown = true
